...
const savingsLedgerSchema = readJson("schemas/turba-savings-ledger.v1.schema.json");
const deviceFlopsCatalog = readJson("schemas/turba-device-flops.v1.json");
const benchmarkContributionSchema = readJson("schemas/turba-benchmark-contribution.v1.schema.json");
...
const redfishSnapshot = readJson("fixtures/redfish-source-snapshot.json");
...
assert.equal(savingsLedgerSchema.$id, "https://turba.analytics/schemas/turba-savings-ledger.v1.schema.json");
assert.equal(deviceFlopsCatalog.schemaVersion, "turba.device_flops.v1");
assert.ok(deviceFlopsCatalog.devices["H100 SXM"].bf16 > 0);
assert.ok(deviceFlopsCatalog.devices.MI300X.fp8 > 0);
assert.equal(benchmarkContributionSchema.properties.schemaVersion.const, "turba.benchmark_contribution.v1");
assert.ok(benchmarkContributionSchema.required.includes("benchmarkOptIn"));
assert.equal(benchmarkContributionSchema.additionalProperties, false);
assert.equal(benchmarkContributionSchema.properties.features.additionalProperties, false);
...
assert.ok(workspaceSchema.properties.savingsLedger.items.$ref.includes("turba-savings-ledger.v1.schema.json"));
assert.ok(savingsLedgerSchema.properties.status.enum.includes("verified"));
assert.ok(savingsLedgerSchema.properties.attribution.enum.includes("measured"));
assert.ok(savingsLedgerSchema.properties.attribution.enum.includes("modeled"));
assert.ok(savingsLedgerSchema.properties.scope.properties.type.enum.includes("tenant"));
...
assert.ok(Array.isArray(sourceFixture.sources.redfish));
...
assert.ok(sourceBundleSchema.$defs.sourceExports.properties.redfish.items.$ref.includes("redfishSample"));
...
assert.ok(sourceBundleSchema.$defs.redfishSample.required.includes("runId"));
assert.ok(sourceBundleSchema.$defs.redfishSample.properties.systems);
assert.ok(sourceBundleSchema.$defs.redfishSample.properties.chassis);
assert.ok(sourceBundleSchema.$defs.redfishSample.properties.health);
...
assert.equal(redfishSnapshot.serviceRoot.RedfishVersion, "1.20.0");
