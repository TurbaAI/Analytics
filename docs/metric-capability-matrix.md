...
| GB10 NVML/gpustat/nvidia-smi | `gb10-nvml-nvidia-smi`, `gpuSource`, `gpuBackendRequested`, GPU source context | `native_nvml` | On GB10 hosts, `gpustat --json` is the preferred lightweight JSON path when installed, while the selective `nvidia-smi` query remains the fallback for inventory, utilization, power, temperature, PCIe, and process data. |
