...
When showing `http://192.168.10.101:8000/`, position it as an observed two-host lab demo: NUC14E has the RTX 4090 and Ollama/Grafana/Netdata/node-exporter present, while SPARK1 is a second Linux host whose NVIDIA telemetry availability is reported exactly as observed. There is no active Kubernetes/DCGM/eBPF/scheduler/provider stack and no fabricated multi-node queue or billing/SLO overlay. The browser refreshes the live-machine bundle every 1 second while visible; if the RTX 4090 is idle or SPARK1 cannot expose GPU counters, say that plainly rather than turning it into a workload bottleneck. Use `?demo=sample` if you need to return to the seeded provider fixture.
