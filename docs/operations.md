...
kubectl -n turbalance-demo get pods,svc -l turba.ai/demo=spark1-kafka
kubectl -n turbalance-demo logs job/spark1-kafka-smoke
```

The helper applies `ops/kubernetes/spark1-kafka.yaml`, waits for `deployment/spark1-kafka`, recreates the smoke Job from `ops/kubernetes/spark1-kafka-smoke-job.yaml`, and fails unless a produced JSON message is consumed back from Kafka. The cluster bootstrap address is `spark1-kafka.turbalance-demo.svc.cluster.local:9092`; the SPARK1 lab NodePort address is `192.168.10.20:30992`.
