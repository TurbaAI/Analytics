...
For the SPARK1 single-node `k3s` demo, run the labeled workload and local collector:
...
The collector reads `kubectl` pod/job/event/node state and queries Prometheus/DCGM when reachable. It emits a source bundle with `sources.kubernetes`, `sources.scheduler`, and optional `sources.prometheus` and `sources.dcgm`; it does not synthesize multi-node topology or provider billing/SLO context.

Kafka can run as an observed SPARK1 service for integration demos. Apply the single-node KRaft broker and run the smoke test with:

```sh
node scripts/check-spark1-kafka.js
```

That helper verifies Kafka functionality by creating a topic, producing a JSON message, and consuming the same message ID back through `spark1-kafka.turbalance-demo.svc.cluster.local:9092`. Kafka reachability is surfaced by `scripts/collect-local-machine-bundle.js` as an observed service when `192.168.10.20:30992` is answering, but Kafka is not a source-bundle lane unless a provider exports approved Kafka metrics or events separately.
