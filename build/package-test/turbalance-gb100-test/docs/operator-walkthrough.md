...
- Use the optional backend ingestion service when a pilot needs signed uploads, tenant isolation, JWKS/JWT tenant mapping, role-aware access, metrics, audit export, token/key rotation, and retention policy before the dashboard imports the accepted source bundle.
